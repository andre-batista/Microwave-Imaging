...
expname             = 'basic_exp04_03_3objects';
geometryname        = '3objects';           % Geometry name
...
epsilon_robj        = [1.5,2.0,1.8];        % Object relative permittivity
sigma_obj           = [0,0,0];              % Object conductivity [S/m]
ra                  = 5.25e-2;              % Circle radius [m]
dela                = [8.25e-2,2.25e-2];    % Cicle position [m]
lb                  = 10.5e-2;              % Triangle size [m]
delb                = [-4.5e-2,-7.5e-2];    % Triangle position [m]
lc                  = 12e-2;                % Square size [m]
delc                = [-6e-2-lc/2*...       % Square position [m]
                        sqrt(3)/2,7.5e-2];
frequency           = [8e8,9e8,1e9,1.5e9,2e9];% Frequency of measurements [Hz]
...
sfi                 = {1:5};                % Set of frequency indexes
misf                = 10;                   % Number of iterations for sets
alpha               = 1e3;                  % Tikhonov regularization
...
parameters = setparameters([min([epsilon_rb,epsilon_robj]),...
    max(epsilon_robj)],[],sfi,misf,'alpha',alpha,'initialization',...
    initialization);
fprintf('ok!\n')

% Set original contrast map
fprintf('Geting testbench map... ')
testbench = getmap(geometryname,finemodel,epsilon_robj,sigma_obj,ra,dela,lb,delb,lc,delc);
fprintf('ok!\n')
